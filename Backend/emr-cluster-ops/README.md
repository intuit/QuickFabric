...
2. Fetch AWS credentials using aws-credentials-helper and update your aws profile (Reference: [eiamCLI](https://docs.aws.amazon.com/cli/latest/userguide/cli-configure-files.html))
  ```
  ➤ $ aws-credentials-helper login
  Username:av1
  Password:
  ...
  Note: 'getKeys.sh' script in the backend calls "aws-credentials-helper getAWSTempCredentials" per AWS account ID
...
3. Goto ```terraform``` directory  
  ...
  ```
  docker run --rm -it -v $(pwd):$(pwd) -v ${HOME}/.aws:/root/.aws -w $(pwd) <your-registry>/terraform:version1.0 plan -var env=dev -var account_id=123456789012 -var aws_profile=sbg-data-processing-dev -state=statefiles/terraform.tfstate.dev.5591-6688-1778
  ```  
  ii. __If you are looking to update on an existing AWS account__ (target specific resource you want to update)
  ```
  docker run --rm -it -v $(pwd):$(pwd) -v ${HOME}/.aws:/root/.aws -w $(pwd) <your-registry>/terraform:version1.0 plan -target=aws_iam_policy.ibp-policy -var env=dev -var account_id=123456789012 -var aws_profile=sbg-data-processing-dev -var sts_externalId=XXXX -state=statefiles/terraform.tfstate.dev.5591-6688-1778
  
  ```  

4. __Apply__ changes  
  a. Generate sts externalId (Example: Use [UUID Generator](https://www.uuidgenerator.net/))  
    i. Generated externalId needs to be added to IBP credentials (https://your-ci.example.com) with ID as 'extId_$accountId' i.e. extId_123456789012 & corresponding secret value  
    ii. And then, reference it in Jenkinsfile. E.g. https://your-internal-repo.example.com  
  b. __Note__: To create cert for API g/w via ACM, use Negenie automation: [AWS SSL Cert Manager](https://your-wiki.example.com)  
  c. Terraform tfstate file is written as ```terraform.tfstate.${env}.${aws_accountId}```. These state files are __encrypted__ using [git-crypt](https://github.com/AGWA/git-crypt) and stored on git to mask API g/w key & STS externalId. To decrypt locally for usage contact <your-team-contact@example.com> for crypt key and then, run in your $GIT_HOME:
  ```
  ...
  ```
  docker run --rm -it -v $(pwd):$(pwd) -v ${HOME}/.aws:/root/.aws -w $(pwd) <your-registry>/terraform:version1.0 apply -var env=dev -var account_id=123456789012 -var aws_profile=sbg-data-processing-dev -var sts_externalId=XXXX -state=statefiles/terraform.tfstate.dev.5591-6688-1778
  ```  
  - __For an existing AWS account__  (target specific resource you want to update)
  ```
  docker run --rm -it -v $(pwd):$(pwd) -v ${HOME}/.aws:/root/.aws -w $(pwd) <your-registry>/terraform:version1.0 apply -target=aws_dynamodb_table.emr_metadata_table -var env=dev -var account_id=123456789012 -var aws_profile=sbg-data-processing-dev -state=statefiles/terraform.tfstate.dev.5591-6688-1778
  ```  
...
```
docker run --rm -it -v $(pwd):$(pwd) -v ${HOME}/.aws:/root/.aws -w $(pwd) <your-registry>/terraform:version1.0 destroy -var aws_profile=sbg-data-processing-dev -state=statefiles/terraform.tfstate.dev.5591-6688-1778
```  
...
8. Push generated API key via serverless into SSM param store  
  ...
  ```
  docker run --rm -it -v $(pwd):$(pwd) -v ${HOME}/.aws:/root/.aws -w $(pwd) <your-registry>/terraform:version1.0 apply -var env=dev -var aws_profile=sbg-data-processing-dev -var sts_externalId=XXXX -var emr_api_key=XXXX -var ssm_param=1 -state=statefiles/terraform.tfstate.dev.5591-6688-1778
  ```  

9. Additionally, the API url/token needs to be pushed to IDPS
a. Push key/value to your IDPS endpoint using [stash](https://your-wiki.example.com)
__Note__: Key names should be: 'registry/api_url' and 'registry/api_token' (this gets referenced in IDPS util: https://your-internal-repo.example.com)
b. Next, add your IDPS endpoint & policyId configs here: https://your-internal-repo.example.com
Example: https://your-internal-repo.example.com
...
➤ $ aws iam get-role --role-name emr-launch-cluster-prod-us-west-2-lambdaRole --region us-west-2 --profile sbg-data-processing-prd
{
    "Role": {
        ...
        "RoleId": "AROAEXAMPLE000000",
        "CreateDate": "2019-04-05T10:57:40Z",
        "RoleName": "emr-launch-cluster-prod-us-west-2-lambdaRole",
        "Path": "/",
        "Arn": "arn:aws:iam::123456789012:role/emr-launch-cluster-prod-us-west-2-lambdaRole"
    }
}
```
b. Where to add?
- https://your-internal-repo.example.com
...
- Example: https://your-jira.example.com
- Additionally, ensure OIL team provides a log subscription URL per account and add it here: https://your-internal-repo.example.com

13. Create EMR cluster
  - __SBSEG Analytics Production__ account: https://your-ci.example.com
  - __SBG Data Processing Platform__ account: https://your-ci.example.com (Scheduled Kerbirized)

14. Create EMR gateway node
  - https://your-ci.example.com  

15. EMR cluster operations
  - Terminate cluster: https://your-ci.example.com
  - Add custom step: https://your-ci.example.com
