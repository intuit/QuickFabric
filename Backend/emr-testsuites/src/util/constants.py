...
DEFAULT_SCRIPT_STEP_NAME = 'Custom Script Name'
...
SPARK_SUBMIT_COMMAND = '/usr/bin/spark-submit'
...
# list-clusters
LIST_CLUSTERS_ACTIVE_STATES = ['STARTING', 'BOOTSTRAPPING', 'RUNNING', 'WAITING', 'TERMINATING']
...
STEP_TYPE_SHELL = ['shell', 'shellscript', 'sh', 'bash']

# Test suite program name
DEFAULT_TEST_CUSTOM_JAR_STEP_NAME = 'Test Custom JAR'
DEFAULT_TEST_SCRIPT_STEP_NAME = 'Test Custom Script Name'
DEFAULT_TEST_STREAMING_STEP_NAME = 'Test Streaming program'
DEFAULT_TEST_HIVE_STEP_NAME = 'Test Hive program'
DEFAULT_TEST_PIG_STEP_NAME = 'Test Pig program'
DEFAULT_TEST_IMPALA_STEP_NAME = 'Test Impala program'
DEFAULT_TEST_SPARK_STEP_NAME = 'Test Spark application'


# Test Suite args
DEFAULT_TEST_SPARK_ARGS = ['--deploy-mode', 'cluster', '--class', 'org.apache.spark.examples.JavaWordCount',
                           '/usr/lib/spark/examples/jars/spark-examples_2.11-2.4.3.jar',
                           's3://support.elasticmapreduce/spark/examples/wordcountdata']

DEFAULT_TEST_HIVE_ARGS = ['-f', 's3://elasticmapreduce/samples/hive-ads/libs/model-build.q', '-d',
                          'INPUT=s3://elasticmapreduce/samples/hive-ads/tables', '-d',
                          'LIBS=s3://elasticmapreduce/samples/hive-ads/libs']


CUSTOM_TEST_JAR = 's3n://elasticmapreduce/samples/cloudfront/logprocessor.jar'

CUSTOM_JAR_TEST_ARGS = ['-input', 's3n://elasticmapreduce/samples/cloudfront/input', '--output',
                   '/tmp/testlog/', '-start', '2019-01-01-14', '-end', '2019-01-02-14',
                    '-tempOutput']

CUSTOM_JAT_TEST_ARGS_OUTPUT_PREFIX = '/home/hadoop/temp-output-'